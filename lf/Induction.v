...
(** **** Exercise: 2 stars, advanced, especially useful (add_comm_informal)
    ...
    - _Theorem_: For any [n] and [m],
      ...
      - Next, suppose [n = S n'], where
        ...
        We can rewrite the right-hand side using the plus_n_Sm theorem which can also be proven by induction. Now we have

          S (n' + m) = S (m + n').
        
        This is immediate from the induction hypothesis. _Qed_.
*)
...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal)
    ...
    - _Theorem_: For any [n],
      ...
      - Next, suppose [n = S n'], where
        ...
        By the definition of [=?], this follows from the induction hypothesis. _Qed_.
*)
...
(** In the [bin_nat_bin_fails] theorem, there may have preceded B0 in b.
    For example, [B1 (B0 Z)] and [B1 Z] are equivalent in semantics, but not in syntax.
    The [nat_to_bin] function will always return the simplest form of a [bin],
    which is the one without any leading [B0]s. *)
...
Example test_normalize0_0 : normalize Z = Z.
Proof. reflexivity. Qed.

Example test_normalize0_1 : normalize (B0 Z) = Z.
Proof. reflexivity. Qed.

Example test_normalize0_2 : normalize (B0 (B0 Z)) = Z.
Proof. reflexivity. Qed.

Example test_normalize0_3 : normalize (B0 (B0 (B0 Z))) = Z.
Proof. reflexivity. Qed.

Example test_normalize1 : normalize (B1 (B0 Z)) = B1 Z.
Proof. reflexivity. Qed.

Example test_normalize2 : normalize (B0 (B1 Z)) = B0 (B1 Z).
Proof. reflexivity. Qed.

Example test_normalize3 : normalize (B1 (B1 (B0 (B0 Z)))) = B1 (B1 Z).
Proof. reflexivity. Qed.

Example test_normalize4 : normalize (B0 (B0 (B1 Z))) = B0 (B0 (B1 Z)).
Proof. reflexivity. Qed.
