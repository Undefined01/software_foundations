...
(** **** Exercise: 1 star, standard, optional (combine_checks)
    ...
    - What is the type of [combine] (i.e., what does [Check
      ...
      Answer: forall X Y : Type, list X -> list Y -> list (X * Y)
...
(* When trying to define [rev] in terms of [fold], [Y] is [list X].
...
(* _Proof_: By induction on [l].
   ...
   - Case [l = []]. The conclusion follows directly from the definition of
     [nth_error].

   - Case [l = a :: l']. The hypothesis [length (a :: l') = n] implies
     that [n = S m] for some [m]. By the definition of length, we have
     that [length l' = m]. By the induction hypothesis, we have that
     [nth_error l' m = None]. By the definition of [nth_error], we have
     that [nth_error (a :: l') n = nth_error l' (pred n)]. _QED_. *)
...
(** First, let's see what happens if we try to define [exp] by iterating [mult n] by [m] times:

      Definition exp (n m : cnat) : cnat :=
        fun (X : Type) (f : X -> X) (x : X) =>
          m cnat (mult n) f x.

    This will cause a error message like
    
      The term "cnat" has type "Type@{cnat.u0+1}" while it is expected to have type "Type@{cnat.u0}"

      (universe inconsistency: Cannot enforce cnat.u0 < cnat.u0 because cnat.u0 = cnat.u0).
    
    This is because we are passing [cnat] itself as the [X] type argument of [m : cnat], which will
    cause a paradox. (Think of it as a set containing itself.)
    Coq tries to assign each [cnat] with a different universe level to avoid this paradox, but it fails.

    To fix this, we can use a different type for the type argument of [m].
    Observe that [X] is already introduced by the [fun] keyword,
    We can use [X -> X] instead of [cnat] as the type argument of [m].
        *)

Definition mult' {X : Type} (n : cnat) (m : X -> X) : X -> X :=
  n X m.
Definition exp' (n : cnat) (m : cnat) : cnat :=
  fun (X : Type) (f : X -> X) (x : X) =>
    m (X -> X) (mult' n) f x.

(** By inlining the definition of [mult], we have a shorter
    definition of [exp]. Which is exactly the same as the form in lambda calculus *)
